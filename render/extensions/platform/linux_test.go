...
`
	...
	expected.Version = "7"
	expected.Like = []string{"rhel", "fedora"}
...
func ComparePlatform(t *testing.T, expected Platform, platform Platform) {
	...
	if reflect.DeepEqual(expected.Like, platform.Like) == false {
		t.Errorf("ParseLSBContent Like wanted %q got %q\n", expected.Like, platform.Like)
	}
