...
func New(localPath, systemPath string) *Keystore {
	return &Keystore{
		LocalPath:  localPath,
...
// Default returns a keystore backed by the default local and system paths.
func Default() *Keystore {
	localPath := ""

	usr, err := user.Current()
	if err == nil {
		localPath = filepath.Join(usr.HomeDir, ".converge/trustedkeys")
	}
...
func (ks *Keystore) StoreTrustedKey(pubkeyBytes []byte) (string, error) {
	if err := os.MkdirAll(ks.LocalPath, 0755); err != nil {
		return "", err
	}
	...
	trustedKeyPath := path.Join(ks.LocalPath, fmt.Sprintf("%x", pubKey.Fingerprint))
...
func (ks *Keystore) DeleteTrustedKey(fingerprint string) error {
	return os.Remove(path.Join(ks.LocalPath, fingerprint))
}
...
func (ks *Keystore) MaskTrustedSystemKey(fingerprint string) (string, error) {
	dst := path.Join(ks.LocalPath, fingerprint)
...
func (ks *Keystore) loadKeyring() (openpgp.KeyRing, error) {
	...
	for _, p := range []string{ks.SystemPath, ks.LocalPath} {
