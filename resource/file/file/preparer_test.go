...
func TestVaildPreparer(t *testing.T) {
	...
	preparers := []resource.Resource{
		//Test default state
		&file.Preparer{File: "/path/to/file", Mode: "0777", User: "nobody"},
		//Test explicit file state
		&file.Preparer{Directory: "/path/to/dir", Mode: "0777", User: "nobody"},
		//Test absent state
		&file.Preparer{Directory: "/path/to/dir", File: "/path/to/dir/file", Mode: "0777", User: "nobody"},
	}
	errs := []string{
		"",
		"",
...
func TestInVaildPreparer(t *testing.T) {
	...
	errs := []string{
		"resource requires either a `directory` or a `file` parameter",
	}
	helpers.PreparerValidator(t, preparers, errs)
}
