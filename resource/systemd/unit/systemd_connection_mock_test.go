...
func (m *SystemdMock) EnableUnitFiles(files []string, runtime, force bool) (bool, []dbus.EnableUnitFileChange, error) {
	args := m.Called(files, runtime, force)
	return args.Bool(0), args.Get(1).([]dbus.EnableUnitFileChange), args.Error(2)
}

func (m *SystemdMock) DisableUnitFiles(files []string, runtime bool) ([]dbus.DisableUnitFileChange, error) {
