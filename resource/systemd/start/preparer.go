...
func (p *Preparer) Prepare(render resource.Renderer) (resource.Task, error) {
	...
	startModule := &Start{Unit: unit, Mode: m, Timeout: t}
	return startModule, ValidateTask(startModule)
}

func ValidateTask(startModule *Start) error {
	if startModule.Unit == "" {
		return errors.New("resource requires a `unit` parameter")
	}
	switch startModule.Mode {
	...
	case common.SMIgnoreRequirements:
		break
	default:
		return fmt.Errorf("resource's parameter 'mode' is not one of %s, found %s", common.ValidStartModes, startModule.Mode)
	}
	return nil
}
